    ...
    <header className="w-full backdrop-blur-sm md:z-[999] bg-transparent md:bg-opacity-60 ">
      <div
        className={`  navbar overflow-hidden   items-center h-[2.5rem] md:h-[13.5rem] lg:h-[7rem] md:bg-transparent lg:bg-transparent ${
          navi ? "bg-[#BCAA99] rounded-bl-none rounded-br-none" : null
        }`}
      >
        <div className=" md:items-center  md:justify-center  px-3 md:px-7 ">
          <div className="flex w-full   justify-end items-center font-extrabold">
            <Link to="/login">
              <span className="md:cursor-pointer  md:text-sm show">
                Login |
              </span>
            </Link>
            <Link to="/register">
        ...
        <div className="flex  justify-between items-center  text-left text-xl px-8  ">
          <div className="flex items-center w-full md:justify-center md:gap-3 gap-2">
            <IoDiamond className="lg:text-4xl md:text-3xl gsap" />
            <motion.h1
              initial={{ y: -100, opacity: 0, scale: 0.7 }}
              animate={{ scale: 1, opacity: 1, y: 0 }}
              transition={{ ease: "linear", duration: 1 }}
              className="webName lg:text-4xl md:text-2xl gsap"
            >
              MOHAN LAL SONS
            </motion.h1>
            <motion.h1
              ...
              animate={{ scale: 1, opacity: 1, y: 0 }}
              transition={{ ease: "linear", duration: 1 }}
              className="md:text-4xl md:hidden gsap"
            >
              MLS
            </motion.h1>
          </div>
          <div className="flex justify-center items-center gap-6">
            <div className="p-2 relative flex items-center justify-center">
              <div className="absolute bg-[#BCAA99] rounded-full p-1 px-2 -right-2 -top-1">
      ...
      >
        ...
        {showButtons && (
          <div className=" flex w-full ">
            <Link to="/login" className="w-[50%] bg-[#BCAA99]">
